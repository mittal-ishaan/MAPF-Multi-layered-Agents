...
class Animation:
    def __init__(self, my_map, inbound_agents, outbound_agents, paths):
        ...
        self.goals = []
        for goal in inbound_agents + outbound_agents:
            gl = []
        ...
        # for i, start in enumerate(self.goals):
        ...
        for i in range(len(self.paths)):
            ...
            for j in range(len(self.goals[i])):
                self.agents[i] = Circle((self.goals[i][j][0], self.goals[i][j][1]), 0.25, facecolor=Colors[i % len(Colors)], edgecolor='#ecf0f1', lw=2)
            self.agents[i].original_face_color = Colors[i % len(Colors)]
            self.patches.append(self.agents[i])

            # Add agent number text with shadow for readability
            text = self.ax.text(self.goals[i][j][0], self.goals[i][j][1] + 0.25, name, fontsize=12, color='white')
