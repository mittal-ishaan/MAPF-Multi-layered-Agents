...
def get_new_goal(layout, current_position):
    ...
    while layout[goal[0]][goal[1]] == 1 or goal == current_position:
        goal = (random.randint(1, HEIGHT - 2), random.randint(1, WIDTH - 2))
    return goal
...
def visualize_movement(layout, goals: list):
    ...
    for i in range(len(paths)):
        robots.append((paths[i][0], goals[i][0])) 
    while True:  # Run indefinitely
        ...
        for i, [goal] in enumerate(goals):
            if paths[i]:
                ...
                for pos in paths[i]:
                    ax.plot(pos[1], pos[0], marker='o', color=robot_colors[i], markersize=1, alpha=0.5)
            ...
            if len(paths[i]) == 1:
                ...
                # Assign a new goal
                new_goal = get_new_goal(layout, paths[i])
                goals[i] = [new_goal]
                paths = cbs.find_extended_solution(i,new_goal, paths)
        ...
        plt.pause(2)  # Pause for 0.3 seconds for the animation effect
